# Event based Delta lake and loading real time cache such as Azure Synapse Analytics
...
The reason why we are choosing to load data into Azure Synapse Analytics is to tackle real time queries from various API that interface to various other business application and also to provide close to real time data as possible.
...
## Now Part 2  Create a data load to read from delta lake and push to azure Synapse Analytics
...
val checkpointLocationforsqldw = "wasbs://xxxx@xxxxxx.blob.core.windows.net/deltaidi/delta_custdata/_checkpoints/etl-from-json_sqldw"
```


to reprocess the data again make sure delete the checkpoint folder
```
dbutils.fs.rm("wasbs://xxxx@xxxxxx.blob.core.windows.net/deltaidi/delta_custdata/_checkpoints/etl-from-json_sqldw", true)
```
...
```
val jdbcconn = "jdbc:sqlserver://xxxxxxx.database.windows.net:1433;database=database;user=adminsuer@servername;password=xxxxxxx;encrypt=true;trustServerCertificate=false;hostNameInCertificate=*.database.windows.net;loginTimeout=30;"
```
...
messages.writeStream
  ...
  .option("url", jdbcconn)
  .option("tempDir", "wasbs://xxxxx@xxxxxx.blob.core.windows.net/stage")
  ...
  Now data will be flowing to Azure Synapse Analytics.